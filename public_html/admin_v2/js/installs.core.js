...
(function (w) {
  ...
  w.buildWeek = w.buildWeek || function buildWeek(startDate, label) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      day.innerHTML = `
  <span class="date" style="background-color:${dateBgColor}" onclick="console.log('Date clicked'); window.toggleDayMaps(this)" title="Click to show/hide map locations">
...
`;
      ...
      todaysInstalls.forEach(ev => {
        ...
        if (label === 'current') {
          ...
          div.innerHTML = `
            <h3><span onclick="editInstall('${ev.id}')" ${ev.fully_paid ? ' class="paid"' : ''}> ${ev.id.replace('-', '<br>')}</span><a class="name-link" href="tel:${ev.phone}">${ev.name || 'No Name'}</a></h3>
  ...
  function ensureProfitToggle() {
    const role = getCookie('auth_role');
    if (role !== 'admin') return;
    if (!w.profitToggleBtn) {
      w.profitToggleBtn = document.createElement('button');
      w.profitToggleBtn.type = 'button';
      w.profitToggleBtn.className = 'profit-toggle-btn';
      w.profitToggleBtn.innerHTML = '<i class="fas fa-eye"></i>';
      w.profitToggleBtn.setAttribute('aria-pressed', String(w.showProfitLines));
      w.toolbarRight.appendChild(w.profitToggleBtn);
      w.apiCounterSpan = document.createElement('span');
      w.apiCounterSpan.className = 'api-counter';
      w.apiCounterSpan.style.display = 'none';
      w.apiCounterSpan.style.marginLeft = '8px';
      w.apiCounterSpan.style.fontSize = '12px';
      w.apiCounterSpan.textContent = 'API: 0';
      w.toolbarRight.appendChild(w.apiCounterSpan);
    }
    w.profitToggleBtn.onclick = () => {
      w.showProfitLines = !w.showProfitLines;
      w.profitToggleBtn.innerHTML = w.showProfitLines ? '<i class="fas fa-eye-slash"></i>' : '<i class="fas fa-eye"></i>';
      w.profitToggleBtn.setAttribute('aria-pressed', String(w.showProfitLines));
  ...
  function updateProfitLineVisibility() {
    ...
    events.forEach(evDiv => {
      ...
      const ev = (w.allInstalls || []).find(x => x.id === id);
